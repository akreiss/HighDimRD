...
#'   uniform.
#' @param alpha,M,L Parameters for "LV" method for tuning parameter choice, ignored
#'   if tpc!="LV".
...
#'     \tabular{ll}{
#'     \code{rd} \tab Output from rdrobust using the selected covariates, see
#'     rdrobust for a description of the output, the estimator can be found in
#'     rd$Estimate. \cr
#'     \code{covs} \tab Vector of selected covariate indices. \cr
#'     \code{b} \tab Bandwidth used for model selection (as returned from
#'     rdbwselect without covariates if b=NULL). \cr
#'     \code{h} \tab Bandwidth used for the estimation (as returned from
#'     rdbwselect with the selected covariates if h=NULL). \cr
#'     \code{Z} \tab pars Vector of length p containing the estimated parameters
#'     of the covariates.}
#'
#'
#' @section Authors:
...
#'
#' @section References: 
#' Belloni, A., Chernozhukov, V. and Hansen, C. (2013): Inference on treatment
...
#'
#' Lederer, J. and Vogt, M. (2020): Estimating the Lasso's Effective Noise. arxiv
#' 2004.11554
#' 
...
HighDim_rd <- function(Y,X,Z,c=0,b=NULL,bfactor=1,h=NULL,tpc,kernel="triangular",alpha=0.05,M=NULL,L=100,OPC=50) {
  ...
  if(is.null(b)==TRUE) {
    b <- rdrobust::rdbwselect(Y,X,c=c,bwselect="mserd",kernel=kernel)
  }
  ...
  if(is.null(h)==TRUE) {
    if(length(sig_cov)==0) {
      ...
      h <- rdrobust::rdbwselect(Y,X,covs=Z[,sig_cov],c=c,bwselect="mserd",kernel=kernel)
    }
  }
  
  ## Estimation
  if(length(sig_cov)==0) {
    RDfit <- rdrobust::rdrobust(Y,X,c=c,h=h$bws[1],b=h$bws[1],kernel=kernel)
  } else {
    RDfit <- rdrobust::rdrobust(Y,X,c=c,h=h$bws[1],b=h$bws[1],covs=Z[,sig_cov],kernel=kernel)
  }
